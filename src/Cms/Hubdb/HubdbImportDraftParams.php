 ...
 * Example usage:
 * ```
 * $params = (new HubdbImportDraftParams); // set properties as needed
 * $client->cms.hubdb->importDraft(...$params->toArray());
 * ```
 ...
 *   `$client->cms.hubdb->importDraft(...$params->toArray());`
 *
 * @see HubspotSDK\Cms\Hubdb->importDraft
 *
 * @phpstan-type hubdb_import_draft_params = array{config?: string, file?: string}
 */
final class HubdbImportDraftParams implements BaseModel
{
    /** @use SdkModel<hubdb_import_draft_params> */
    use SdkModel;
    use SdkParams;
