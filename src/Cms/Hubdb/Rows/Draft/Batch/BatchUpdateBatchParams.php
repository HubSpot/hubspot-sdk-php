 ...
 * Example usage:
 * ```
 * $params = (new BatchUpdateBatchParams); // set properties as needed
 * $client->cms.hubdb.rows.draft.batch->updateBatch(...$params->toArray());
 * ```
 ...
 *   `$client->cms.hubdb.rows.draft.batch->updateBatch(...$params->toArray());`
 *
 * @see HubspotSDK\Cms\Hubdb\Rows\Draft\Batch->updateBatch
 *
 * @phpstan-type batch_update_batch_params = array{
 *   inputs: list<HubDBTableRowV3BatchUpdateRequest>
 * }
 */
final class BatchUpdateBatchParams implements BaseModel
{
    /** @use SdkModel<batch_update_batch_params> */
    use SdkModel;
    use SdkParams;
